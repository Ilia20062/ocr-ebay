...
export const GET = withAuth(async (_req, userId, params) => {
  ...
  const finalCode =
    ...
    (result.auto_approved ? result.extracted_code : null);
...
export const PATCH = withAuth(async (req, userId, params) => {
  ...
  debug(
    `Action: ${action}${manual_override ? `, override: "${manual_override}"` : ""}`,
  );
  ...
  const ocrResult = rawOcr as unknown as
    | (OcrResult & { images: { user_id: string } })
    | null;
  ...
  debug(
    `OCR result found: extracted_code="${ocrResult.extracted_code}", image_id=${ocrResult.image_id}`,
  );
  ...
  const finalCode =
    action === "override" ? manual_override! : ocrResult.extracted_code;
  debug(`Final code for search: "${finalCode}"`);

  let searchResult: "found" | "not_found" | "no_code" | "search_error" =
    "no_code";
  ...
  if (!finalCode) {
    ...
    try {
      ...
      const { data: search, error: searchInsertErr } = await db
      ...
      if (searchInsertErr || !search) {
        debug(
          `Failed to create product_search record: ${searchInsertErr?.message ?? "unknown"}`,
        );
        ...
        if (!conn) {
          debug(
            "❌ No eBay connection found for this user! Cannot search or list.",
          );
          searchResult = "search_error";
          await db
            .from("product_searches")
            .update({ status: "failed", error_message: "No eBay connection" })
            .eq("id", search.id);
        } else {
          debug(
            `eBay connection found: ebay_user_id=${conn.ebay_user_id}, token_expires_at=${conn.token_expires_at}`,
          );
          ...
          if (best) {
            debug(
              `Best match: "${best.title}" (${best.itemId}), price=${best.price.value} ${best.price.currency}, condition=${best.condition}`,
            );
            ...
            debug(
              `Auto-listing result: success=${listingResult.success}${listingResult.error ? `, error=${listingResult.error}` : ""}${listingResult.listingUrl ? `, url=${listingResult.listingUrl}` : ""}`,
            );
          } else {
            debug(
              `No matching product found for "${finalCode}" — ${items.length} items returned but none matched`,
            );
  ...
  debug(
    `=== DONE === searchResult=${searchResult}, listingSuccess=${listingResult?.success ?? "N/A"}`,
  );
